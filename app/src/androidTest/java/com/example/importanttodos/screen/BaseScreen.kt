...
open class BaseScreen {
    ...
    fun checkIsDisplayed(text: String) {
        onView(withText(text)).check(matches(isDisplayed()))
    }

    fun checkIsNotDisplayed(text: String) {
        onView(withText(text)).check(matches(not(isDisplayed())))
    }

    fun clearText(@IdRes viewId: Int) {
        onView(withId(viewId)).perform(androidx.test.espresso.action.ViewActions.clearText())
    }

    fun checkViewContainsTextInList(@IdRes listViewId: Int, text: String) {
        onView(allOf(withText(text), isDescendantOfA(withId(listViewId))))
            .check(matches(isDisplayed()))
    }

    fun clickOnViewWithTextInList(@IdRes listViewId: Int, text: String) {
        onView(allOf(withText(text), isDescendantOfA(withId(listViewId))))
            .perform(click())
    }

    fun checkViewDoesNotExistInList(@IdRes listViewId: Int, text: String) {
        // Check that the text does not exist within the specified list/RecyclerView
        onView(allOf(withText(text), isDescendantOfA(withId(listViewId))))
            .check(doesNotExist())
    }

    private fun allOf(vararg matchers: Matcher<View>): Matcher<View> {
        return org.hamcrest.Matchers.allOf(*matchers)
    }

    private fun isDescendantOfA(parentMatcher: Matcher<View>): Matcher<View> {
        return androidx.test.espresso.matcher.ViewMatchers.isDescendantOfA(parentMatcher)
    }
}
